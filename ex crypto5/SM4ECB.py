...
def main():
    ...
    '''with open('pic_original.bmp', 'rb') as file:
        ...
        #numstr = (" {:02X}".format(int(c)) for c in file.read())
    ...
    if op == 1:
        #for i in range(len(numstr) // 32):  # 每32个加密
        for i in range(len(numstr) // 32):  # 每32个加密
            ansline = []
            #for j in range(32):
            for j in range(16):
                ansline.append(Word2hex(sm4_encrypt(eval('0x' + numstr[32 * i:32 * i + 32]), k)))
            ansmat.append(ansline)
        for i in range(len(numstr) // 32):  # maximum注意
            pick = 0xff000000000000000000000000000000
            for j in range(16):
                # if 16*i+j <= max:
                #if 16 * i + j < 160:想当然
                print("0x", end='')
                print("%02x" % ((ansmat[i][j] & pick) >> (120 - 8 * j)), end=' ')
                pick >>= 8
            print()
            '''
        ans = ''
        for i in range(len(numstr) // 32):  # 每32个加密
            # for j in range(32):
            hex_ = hex(Word2hex(sm4_encrypt(eval('0x' + numstr[32 * i:32 * i + 32]), k)))[2:]
            while len(hex_) < 32:
                hex_ = '0' + hex_
            ans += hex_'''
        ...
        for i in range(len(numstr)//32):#每32个加密
            ansline = []
            for j in range(16):
                ansline.append(Word2hex(sm4_decrypt(eval('0x'+numstr[32*i: 32*i + 32]), k)))
            ansmat.append(ansline)
    ...
    #   maximum = 160
        ...
        for i in range(len(numstr) // 32):  # 每32个加密
            # for j in range(32):
            hex_ = hex[2:](Word2hex(sm4_encrypt(eval('0x' + numstr[32 * i:32 * i + 32]), k)))
            while len(hex_) < 32:
                hex_ = '0' + hex_
            ans += hex_
        ans = ''
        ...
        for i in range(len(numstr) // 32):  # 每32个加密
            # for j in range(32):
            if (len(numstr) // 32) * (i + 1) < maximum:
                hex_ = hex[2:](Word2hex(sm4_encrypt(eval('0x' + numstr[32 * i:32 * i + 32]), k)))
                while len(hex_) < 32:
                    hex_ = '0' + hex_
                ans += hex_
            else:
                hex_ = hex(Word2hex(sm4_encrypt(eval('0x' + numstr[32 * i:]), k)))[2:]
                while len(hex_) < 32:
                    hex_ = '0' + hex_
                ans += hex_
    
    with open('output_ECB.txt', 'w') as output_file:
...
#解密还要再去掉 补0
#校验字段检查 也应该是14个0d啊
#怎么读入5+1+15+1+4+10+1+15+1+12+3+1+15+1+15+1+3+12+15+1+10+4+1+10