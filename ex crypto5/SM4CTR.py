#T随便选？与明文分组等长 128bit
#T = [0xf1f2f3f4f5f6f7f8f9fafbfcfdfeff] *
#
...
def enCTR(P, k, IV, l):
    ...
    #T = 0xf1f2f3f4f5f6f7f8f9fafbfcfdfeff
    for i in range(q-1):
        Y = sm4_encrypt(IV + i, k)
        C[i] = P[i] ^ Y
    Y = sm4_encrypt(IV + q-1, k)
    if l % 32 != 0:
        d = l % 32
    else:
        d = 32
...
def deCTR(C, k, IV, l):
    ...
    for i in range(q - 1):
        Y = sm4_encrypt(IV + i, k)
        P[i] = C[i] ^ Y
    Y = sm4_encrypt(IV + q - 1, k) #是否加一个参数
    #d = C[q - 1].bit_length()  # 不会有前导0？ %n 注意整除
    '''if l % 128 != 0:
        d = l % 128
    else:
        d = 128'''
...
def main():
    ...
    '''if op == 1:
        ...
        if len(numstr) % 32 != 0:
            P.append(eval('0x' + numstr[32 * (len(numstr) // 32):]))
    
        #C = enCTR(P, IV, k)#16*2*4 一个
        C = enCTR(P, k, IV)
        ...
        if len(numstr) % 32 != 0:
            C.append(eval('0x' + numstr[32 * (len(numstr) // 32):]))
        P = deCTR(C, k, IV)  # 16*2*4 一个
        l_P = len(P)
        for i in range(l_P):
            pick = 0xff000000000000000000000000000000
            for j in range(16):
    ...
    if op == 1:#error 输出与明文一样
        ...
        for i in range(len(numstr)//32):
            P.append(eval('0x' + numstr[32*i: 32*i+32]))
        if len(numstr) % 32 != 0:
            P.append(eval('0x' + numstr[32*(len(numstr)//32): ])) #注意优先级
        C = enCTR(P, k, IV, len(numstr))#16*2*4 一个
        l_C = len(C)#是否需要讨论整除 最后一个元素的输出
        '''for i in range(l_C):
            pick = 0xff000000000000000000000000000000
            for j in range(16):
        ...
        for i in range(len(numstr) // 32):
            C.append(eval('0x' + numstr[32 * i: 32 * i + 32]))
        if len(numstr) % 32 != 0:
            C.append(eval('0x' + numstr[32 * (len(numstr) // 32):]))  # 注意优先级
        P = deCTR(C, k, IV, len(numstr))  # 16*2*4 一个
