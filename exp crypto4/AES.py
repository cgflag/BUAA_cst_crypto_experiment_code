...
def encrypt(s, key):
    ...
    for i in range(0, 9):
        ...
        c_mat = RM(c)
        if i == 8:
            '''for row in range(len(c_mat)):
                for col in range(len(c_mat[row])):
                    print("%2x"%c_mat[row][col], end=' ')
                print()'''
            #c_mat[0][0] ^= 1
            #c_mat[0][0] ^= 1
            '''for row in range(len(c_mat)):
    ...
    #print("%2x" % (s_box[c_mat[1][0] // 16][c_mat[1][0] % 16] ^ 130), end=' ')
    #print("%2x" % s_box[(c_mat[1][0] ^ gfmutiply(1, 1)) // 16][(c_mat[1][0] ^ gfmutiply(1, 1)) % 16])
...
def decrypt(s, key):
    ekey_mat = key_expand(key)
    cur_key = (ekey_mat[40][0] << 120) | (ekey_mat[40][1] << 112) | (ekey_mat[40][2] << 104) | (ekey_mat[40][3] << 96) | (
                ekey_mat[41][0] << 88) | (ekey_mat[41][1] << 80) | (ekey_mat[41][2] << 72) | (ekey_mat[41][3] << 64) | (
                          ekey_mat[42][0] << 56) | (ekey_mat[42][1] << 48) | (ekey_mat[42][2] << 40) | (
                          ekey_mat[42][3] << 32) | (ekey_mat[43][0] << 24) | (ekey_mat[43][1] << 16) | (
                          ekey_mat[43][2] << 8) | (ekey_mat[43][3])
    #print("%32x" % cur_key)
    c = cur_key ^ s
    i = 0
    #print("%32x" % c)
